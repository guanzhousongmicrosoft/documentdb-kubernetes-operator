...
CHAOS_DELAY_SECONDS="${CHAOS_DELAY_SECONDS:-15}"
...
CHAOS_FILE="$SCRIPT_DIR/ha-failure.yaml"
...
initial_primary_pod=$(kubectl --context "$primary_context" get pods -n "$DOCUMENTDB_NAMESPACE" -l "cnpg.io/instanceRole"="primary" -o jsonpath='{.items[0].metadata.name}')
...
final_primary_pod=$(kubectl --context "$primary_context" get pods -n "$DOCUMENTDB_NAMESPACE" -l "cnpg.io/instanceRole"="primary" -o jsonpath='{.items[0].metadata.name}')
echo "HA Primary Pod changed from $initial_primary_pod to $final_primary_pod"
