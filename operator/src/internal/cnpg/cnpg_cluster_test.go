...
var _ = Describe("GetCnpgClusterSpec", func() {
	...
	It("creates a CNPG cluster spec with default bootstrap", func() {
		...
		Expect(result.Spec.PostgresConfiguration.Extensions[0].ExtensionControlPath).To(Equal([]string{"usr/share/postgresql/18/extension"}))
		Expect(result.Spec.PostgresConfiguration.Extensions[0].LdLibraryPath).To(Equal([]string{"usr/lib/postgresql/18/lib"}))
